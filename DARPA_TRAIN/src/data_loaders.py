...
__all__ = ['CIFAR10DataLoader', 'ImageNetDataLoader','UAVHUMANDataLoader', 'CIFAR100DataLoader']
...
class RandomGaussianBlur(object):
    ...
    Apply Gaussian Blur to the PIL image.
    """
    def __init__(self, p=0.5, kernal_size=7, sigma_min=0.1, sigma_max=2.):
        self.prob = p
    ...
    def __call__(self, clips):
        ...
        T = transforms.Compose([
            transforms.GaussianBlur(self.kernal_size,sigma=(self.sigma_min, self.sigma_max))
        ])
        return T(clips)
...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Union[
        ...
        if self.datasample == 'normal':
            return self._get(record, frame_start_indices)
        else:
            return self._randomget(record, frame_start_indices)
    ...
    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        ...
        if self.transform is not None:
            ...
            # print('The size of the images will be {}'.format(images.size()))

        return images, record.label
    
    
    def _randomget(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        Tuple[List[Image.Image], Union[int, List[int]]],
        Tuple['torch.Tensor[num_frames, channels, height, width]', Union[int, List[int]]],
        Tuple[Any, Union[int, List[int]]],
        ]:
        ...
        # frame_start_indices = frame_start_indices + record.start_frame
        # frame_start_indices = frame_start_indices*4
        # print(frame_start_indices)
        # frame_start_indices = [random.randint(record.start_frame,record.end_frame) for _ in range(self.frames_per_segment*self.num_segments)]
        # frame_start_indices.sort()
        # images = list()
...
class Permute(torch.nn.Module):
    ...
    """
    @staticmethod
    def forward(img_tensor) -> 'torch.Tensor[CHANNELS, NUM_IMAGES, HEIGHT, WIDTH]':

        return img_tensor.permute(1,0,2,3)
...
class UAVHUMANDataLoader(DataLoader):
    def __init__(self, data_dir, split='train', image_size=224, batch_size=16, num_workers=8):
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, split), transform=transform)
        super(UAVHUMANDataLoader, self).__init__(
            dataset=self.dataset,
            batch_size=batch_size,
            shuffle=True if split == 'train' else False,
            num_workers=num_workers)
...
class VIDEODataLoader(DataLoader):
    def __init__(self, datasample, data_dir, split='train',image_size=224, batch_size=16, num_workers=8):
        ...
        self.dataset = VideoFrameDataset(
                ...
                annotationfile_path=os.path.join(data_dir,split+'.txt'),
                num_segments = 16,
...
if __name__ == '__main__':
    ...
    data_loader = VIDEODataLoader(
        data_dir='/fs/nexus-projects/AerialAI/data/UAVHuman/frames',
        split='train',
        image_size=224,
        batch_size=16,
        num_workers=0,)
