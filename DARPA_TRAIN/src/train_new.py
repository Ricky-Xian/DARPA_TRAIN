...
def train_epoch(epoch, model, data_loader, criterion, optimizer, lr_scheduler, metrics, device=torch.device('cpu')):
    ...
    for batch_idx, (batch_data, batch_target) in enumerate(data_loader):
        ...
        acc1, acc5 = accuracy(batch_pred, batch_target, topk=(1, 2))
...
def save_model(save_dir, epoch, model, optimizer, lr_scheduler, device_ids, best=False):
    ...
    filename = str(save_dir + 'current.pt')
    torch.save(state, filename)

    if best:
        filename = str(save_dir + 'best.pt')
        torch.save(state, filename)
...
# python3 src/train_new.py --exp-name darpa_severehamorhhage --n-gpu 8 --tensorboard  --model-arch b16  --image-size 224 --batch-size 64 --data-dir /fs/nexus-projects/AerialAI/DARPA/severe_hamorrhage --dataset UAVHUMAN --num-classes 2 --train-steps 10000 --lr 0.005 --wd 0.0
